(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('spPortalApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-spPortalApp-error');
                var entityKey = httpResponse.headers('X-spPortalApp-params');
                if (errorHeader) {
